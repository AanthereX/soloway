...
import { DeleteProjectApiMethod, getUserRelatedProjectMethod } from "../store/actions/projectActions"
import { useDispatch } from "react-redux"
export default ({open = false,setOpen = () => {},handleProject,setSearch,projectId}) => {
    const cancelButtonRef = useRef(null);
    const dispatch = useDispatch();


    const deleteProjectFunc = async () => {
        await DeleteProjectApiMethod(projectId, () => {
          handleProject();
          setOpen(false);
          setSearch("");
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      ...
                      <div className="mt-2 flex flex-col justify-center items-center">
                        ...
                        <p className=" text-white text-lg">
                          {labels.areYouSureYouWantToDelete}
                        </p>
                      </div>
                    </div>
                  </div>
